...
contamdf.freq2 <- isContaminant(pso_no_double, method="either", conc="quant_reading", neg="is.neg")
table(contamdf.freq2$contaminant)
pso.noncontam <- prune_taxa(!contamdf.freq2$contaminant, pso_no_double)

#########################################Remove samples used as blanks
pso.noncontam <- remove_samples (samples = "AB", pso.noncontam)
pso.noncontam <- remove_samples (samples = "A21", pso.noncontam)
pso.noncontam <- remove_samples (samples = "B21", pso.noncontam)

#########################################Removal of archaeal, chloroplast and mitochondrial reads
pso.nochloro <- pso.noncontam %>% subset_taxa(Order!="Chloroplast" | is.na(Order))
pso.nomito <- pso.nochloro %>% subset_taxa(Family!="Mitochondria" | is.na(Family))
pso.onlybac <- pso.nomito %>% subset_taxa(Kingdom!="Archaea" | is.na(Kingdom))
...
pso.rare.rel.backgr <- remove_samples (samples = "B14", pso.rare.rel.backgr)
pso.rare.rel.backgr <- remove_samples (samples = "B16", pso.rare.rel.backgr)
...
pso.rare.rel.dust <- remove_samples (samples = "A23", pso.rare.rel.dust)
...
pso.rare.rel.dust <- remove_samples (samples = "B31", pso.rare.rel.dust)
...
#########################################plotting barcharts
ps.rare.phylum <- phyloseq::tax_glom(ps.rare, "Phylum", NArm=FALSE)
...
colourCount = length(unique(taxic$Phylum))  
...
ggplot(bars_phylum, 
       aes(x = Date, y = Abundance, fill = Phylum)) + 
  facet_grid(Height~.) +
  theme(legend.title = element_text(size=12),
        legend.text = element_text(size=10))+
  geom_bar(stat = "identity") + scale_fill_manual("Phylum", values = getPalette(colourCount)) +
